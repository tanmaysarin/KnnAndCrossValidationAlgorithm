...
def my_euclidean_dist(X_test, X_train):
    ...
    for i in range(num_test):
        for num in range(num_train):
            x_cord.append(
                (X_test[i][0] - X_train[num][0]) ** 2)  # store the squared difference between each x coordinate
            y_cord.append(
                (X_test[i][1] - X_train[num][1]) ** 2)  # store the squared difference between each y coordinate
    ...
    dists = final.reshape(num_test,
                          num_train)  # reshape the list to get the desired shape of the matrix = (num_test, num_train)
